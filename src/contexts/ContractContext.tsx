...
import { GOVERNANCE_ABI } from "@/abis/governance";
...
// CrowdFunding ABI (deployed contract)
...
export const ContractProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Merge with defaults so new fields like stakingAddress get populated
  ...
  useEffect(() => {
    ...
    try {
      ...
      setTokenContract(
        settings.tokenAddress && tokenAbi.length > 0
          ? new ethers.Contract(settings.tokenAddress, tokenAbi, runner)
          : null
      );
      setCrowdfundingContract(
        settings.crowdfundingAddress && crowdAbi.length > 0
          ? new ethers.Contract(settings.crowdfundingAddress, crowdAbi, runner)
          : null
      );
      setStakingContract(
        settings.stakingAddress && ethers.utils.isAddress(settings.stakingAddress)
          ? new ethers.Contract(settings.stakingAddress, STAKING_ABI, runner)
          : null
      );
      setGovernanceContract(
        settings.governanceAddress && ethers.utils.isAddress(settings.governanceAddress)
          ? new ethers.Contract(settings.governanceAddress, GOVERNANCE_ABI, runner)
          : null
      );
    } catch (error) {
  ...
  useEffect(() => {
    const fetchReadOnly = async () => {
      ...
      try {
        ...
        setTokenSymbol(symbol);
        setTokenDecimals(decimals);
      } catch (e) { console.error("Error fetching read-only token info:", e); }
    };
    fetchReadOnly();
  }, [tokenContract, address]);
  ...
  const refreshTokenBalance = async () => {
    ...
    } catch (error) { console.error("Error refreshing token balance:", error); }
  };
  ...
  const getAllowance = async (): Promise<string> => {
    ...
    } catch (error) { console.error("Error getting allowance:", error); return "0"; }
  };
